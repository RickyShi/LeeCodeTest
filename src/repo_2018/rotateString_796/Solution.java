package repo_2018.rotateString_796;
...
public class Solution {
    ...
    /**
     * Ricky Way use Java function, Don't forget corner case
     */
    public boolean rotateString(String A, String B) {
        if (A.length() != B.length()) return false;
        if (A.equals(B) && A.equals("")) return true;
        for (int i = 1; i < A.length(); i++) {
            String a = A.substring(i) + A.substring(0, i);
            if (a.equals(B)) {
                return true;
            }
        }
     ...
     *
     * KMP Algorithm Details: https://blog.csdn.net/v_july_v/article/details/7041827
